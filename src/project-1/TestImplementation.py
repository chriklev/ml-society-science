...
def utility_from_test_set(X, y, decision_maker, interest_rate):
    ...
    for new_obs in range(num_obs):
        ...
        obs_utility[new_obs] = utility_from_obs(predicted_decision, true_decision, amount, duration, interest_rate)
...
def compare_decision_makers(num_of_tests, response, interest_rate):
    ...
    for i in range(num_of_tests):
        ...
        bank_utility_random[i], bank_investment_random[i] = utility_from_test_set(X_test, y_test, r_banker, interest_rate)
        bank_utility_name[i], bank_investment_name[i] = utility_from_test_set(X_test, y_test, n_banker, interest_rate)

    print(f"Avg. utility [random]= {np.sum(bank_utility_random)/num_of_tests}")
    print(f"Avg. ROI [random] {np.sum(bank_investment_random)/num_of_tests}")
